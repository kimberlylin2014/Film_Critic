...
describe("Form Register Tests", () => {
    ...
    it('Username Input should change value when OnChange is called', ()=> {
        const event = {
            target: {value: "helloo", name: 'username'}
        }
        wrapper.find('[name="username"]').simulate("change", event)
        expect(wrapper.state('username')).toBe('helloo')
    })
